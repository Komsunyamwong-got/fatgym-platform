...
export function MemberProfileView({ member }: { member: any }) {
  // Transform measurement data for charts
  const weightData = member.measurements?.map((m: any) => ({
    month: m.date.toLocaleDateString('en-US', { month: 'short' }),
    sales: m.weight // Map to 'month' and 'sales' so it matches what DashboardCharts expects
  })).reverse() || [];
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
              <span className="flex items-center gap-1"><Phone className="w-4 h-4" /> {member.phone || "N/A"}</span>
              <span className="flex items-center gap-1"><Mail className="w-4 h-4" /> {member.user?.email || "N/A"}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="space-y-6">
          ...
          <Card className="border-none shadow-sm">
            ...
            <CardContent className="space-y-4">
              {member.goals && member.goals.length > 0 ? member.goals.map((goal: any, i: number) => (
        ...
        <div className="lg:col-span-2">
          <Tabs defaultValue="progress" className="space-y-6">
            <TabsList className="bg-card border w-full justify-start h-12 p-1 gap-2">
              ...
              <TabsTrigger value="attendance" className="gap-2 px-6 h-10 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
            ...
            <TabsContent value="sessions" className="space-y-4">
              {member.ptSessions && member.ptSessions.length > 0 ? member.ptSessions.map((session: any, i: number) => (
            ...
            <TabsContent value="attendance" className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-muted/20 rounded-xl mb-4">
                <span className="font-bold text-sm text-muted-foreground">Total Check-ins</span>
                <span className="text-xl font-black text-primary">{member.checkIns?.length || 0}</span>
              </div>
              
              {member.checkIns && member.checkIns.length > 0 ? member.checkIns.map((checkIn: any, i: number) => (
                <div key={i} className="flex items-center justify-between p-4 rounded-xl border bg-card hover:bg-muted/30 transition-colors group shadow-sm">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-bold group-hover:text-primary transition-colors">Gym Check-in</h4>
                      <p className="text-xs text-muted-foreground">{checkIn.createdAt.toLocaleString()}</p>
                    </div>
                  </div>
                  <Badge variant="outline" className="border-none font-bold text-[10px] bg-green-100 text-green-700">
                    COMPLETED
                  </Badge>
                </div>
              )) : (
                <div className="text-center py-12 bg-muted/20 rounded-2xl border border-dashed text-muted-foreground">No attendance history recorded yet.</div>
              )}
            </TabsContent>
          </Tabs>
        </div>
